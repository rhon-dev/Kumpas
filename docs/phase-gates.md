...
| 8 | Session logging / optional backend | ✅ | Tasks 1–15 COMPLETE: SessionDatabase (SQLite schema), SessionManager (lifecycle), DataExporter (JSON export), KumpasChannel Dart methods, SessionRepository wrapper, SessionLifecycleObserver (app lifecycle), PracticeScreen integration, AssessmentScreen (pre/post questionnaires + SUS), export/clear buttons in Settings, retention policy card, unit tests (Tasks 14–15). Task 16 (emulator verification) pending real device testing. |
